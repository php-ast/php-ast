 ...
 */
 ...
 function traverseAndConvert($node) {
     if($node instanceof ast\Node) {
         // 将节点kind值转化为字符串类型，如果它是数字的话
         if(is_numeric($node->kind)) {
             $node->kind = ast\get_kind_name($node->kind);
         }
         if(is_numeric($node->flags)) {
              $node->flags = ast\get_kind_name($node->kind);
         }

         // 递归处理当前节点的所有子节点
         foreach($node->children as $index => $child) {
             $node->children[$index] = traverseAndConvert($child);
         }
     }
     return $node;
 }
